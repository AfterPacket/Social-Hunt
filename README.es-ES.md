...
Para incluir IOPaint detrás del mismo proxy:
...
Terminación HTTPS — enruta `/` a Social-Hunt y `/iopaint` a IOPaint.
...
## Notas sobre proxy inverso (IOPaint en el mismo dominio)

Al ejecutar Social-Hunt e IOPaint detrás del mismo proxy inverso nginx, las solicitudes se dividen por ruta:

- `/` → Social-Hunt (`social-hunt:8000`)
- `/iopaint/` → IOPaint (`iopaint:8080`)
- `/sh-api/` → API de Social-Hunt (`social-hunt:8000`)

El `nginx.conf` en `docker/` maneja esto automáticamente al usar el perfil `nginx` o `ssl`.

> **Importante:** No expongas el prefijo de ruta `/api/` externamente — está reservado para la API interna de IOPaint. Social-Hunt usa `/sh-api/` exclusivamente.
